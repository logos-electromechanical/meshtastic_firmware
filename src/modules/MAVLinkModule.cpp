...
MAVLinkModuleRadio *mavlinkModuleRadio;

#if defined(TTGO_T_ECHO) || defined(CANARYONE) || defined(MESHLINK)
MAVLinkModule::MAVLinkModule() : StreamAPI(&Serial), concurrency::OSThread("Serial") {}
static Print *serialPrint = &Serial;
#elif defined(CONFIG_IDF_TARGET_ESP32C6)
MAVLinkModule::MAVLinkModule() : StreamAPI(&Serial1), concurrency::OSThread("Serial") {}
static Print *serialPrint = &Serial1;
#else
MAVLinkModule::MAVLinkModule() : StreamAPI(&Serial2), concurrency::OSThread("Serial") {}
...
MAVLinkModuleRadio::MAVLinkModuleRadio() : MeshModule("MAVLinkModuleRadio")
{
 ...
 */
bool MAVLinkModule::checkIsConnected()
{
    return Throttle::isWithinTimespanMs(lastContactMsec, SERIAL_CONNECTION_TIMEOUT);
}

int32_t MAVLinkModule::runOnce()
{
    /*
    ...
    */
    ...
    if (moduleConfig.serial.override_console_serial_port || (moduleConfig.serial.rxd && moduleConfig.serial.txd)) {
        if (firstTime) {
            ...
            Serial.setTimeout(moduleConfig.serial.timeout > 0 ? moduleConfig.serial.timeout : TIMEOUT);
#endif
            mavlinkModuleRadio = new MAVLinkModuleRadio();
...
#endif
                    mavlinkModuleRadio->sendPayload();
                }
            }
#endif
        }
 ...
 * @throws None
 */
void MAVLinkModule::sendTelemetry(meshtastic_Telemetry m)
{
 ...
 */
meshtastic_MeshPacket *MAVLinkModuleRadio::allocReply()
{
 ...
 */
void MAVLinkModuleRadio::sendPayload(NodeNum dest, bool wantReplies)
{
 ...
 */
ProcessMessage MAVLinkModuleRadio::handleReceived(const meshtastic_MeshPacket &mp)
{
    if (moduleConfig.serial.enabled) {
        if (moduleConfig.serial.mode == meshtastic_ModuleConfig_SerialConfig_Serial_Mode_PROTO) {
 ...
 */
uint32_t MAVLinkModule::getBaudRate()
{
 ...
 * @return void
 */
void MAVLinkModule::processWXSerial()
{
